import type { AppNotificationOptions } from '../notifications/types'
import type { NormalizedAppError } from './types'
import { normalizeError } from './normalizeError'

interface AppNotifier {
  error: (message: string, options?: AppNotificationOptions) => void
}
...
export async function handleAppError(error: unknown, options: HandleAppErrorOptions = {}): Promise<HandleAppErrorResult> {
  ...
  const shouldNotify =
    options.notifyUser !== false && options.notifier && (options.notifyValidationError || normalized.type !== 'validation')
  if (shouldNotify) {
    options.notifier?.error(normalized.message, {
